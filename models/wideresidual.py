...
class WideResNet(nn.Module):
    def __init__(self, num_classes, block, depth=50, widen_factor=1, preprocessing={}):
        super().__init__()

        self.preprocessing = preprocessing
        if not len(preprocessing) == 0:
            print("mean: ", preprocessing['mean'])
            print("std : ", preprocessing['std'])
    ...
    def forward(self, x):
        if not len(self.preprocessing) == 0:
            # print("normalize within the network!!")
            x = (x - self.mu) / self.sigma
            # torch.save(x, './data/normalization/imagenet32_gpu.pt')
